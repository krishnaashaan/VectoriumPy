...
def test_mirror_formula():
    assert mirror_formula(None,10,10) == 5
    assert mirror_formula(5,10,None) ==10
    assert mirror_formula(5,None,10)==10
    with pytest.raises(ValueError):
        mirror_formula(None,None,10)
       
def test_lens_formula():
    assert lens_formula(None,10,20) == 20
    assert lens_formula(20,None,10) == 20
    assert lens_formula(20,10,None) == 20

    with pytest.raises(ValueError):
        lens_formula(None,None,10)
